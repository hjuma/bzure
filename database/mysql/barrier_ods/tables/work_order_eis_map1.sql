...
-- Dumping structure for table barrier_ods.work_order_eis_map1
CREATE TABLE IF NOT EXISTS `work_order_eis_map1` (
  ...
  `source_extracted_dt` datetime NOT NULL,
  PRIMARY KEY (`BarrierSk`),
  KEY `Order` (`Order`,`current_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Dumping data for table barrier_ods.work_order_eis_map1: ~0 rows (approximately)
