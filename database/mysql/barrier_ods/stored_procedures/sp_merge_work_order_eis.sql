...
-- Dumping structure for procedure barrier_ods.sp_merge_work_order_eis
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_merge_work_order_eis`(
...
BEGIN
	...
	DROP TEMPORARY TABLE IF EXISTS `barrier_ods`.`work_order_eis_tmp`;

	CREATE TEMPORARY TABLE `barrier_ods`.`work_order_eis_tmp` AS
	SELECT `new`.*
	FROM `barrier_ods`.`vw_merged_work_order_eis` `new`
	WHERE 1=0;

	ALTER TABLE  `barrier_ods`.`work_order_eis_tmp`
	ADD COLUMN `action` VARCHAR(1) NOT NULL;


	INSERT INTO `barrier_ods`.`work_order_eis_tmp`
		(
		     `asset_code`
			, `facility_code`
         , `functional_loc`
...
SELECT     `new`.`asset_code`
			...
			, `new`.`coding`
			, 'C' AS 'action'
	FROM `barrier_ods`.`vw_merged_work_order_eis` AS `new`
	INNER JOIN  `barrier_ods`.`merged_work_order_eis` AS `existing`
...
SELECT      `new`.`asset_code`
	...
	AND `new`.`facility_code` = `existing`.`facility_code`
	AND `existing`.`current_flag` = 1
	WHERE `new`.`Order` NOT IN (SELECT `Order` FROM `barrier_ods`.`work_order_eis_tmp`);
...
SELECT
			...
			, 'I' AS 'action'
	FROM `barrier_ods`.`vw_merged_work_order_eis` AS `new`
	LEFT JOIN  `barrier_ods`.`merged_work_order_eis` AS `existing`
	ON `new`.`Order` = `existing`.`order`
	AND `new`.`asset_code` = `existing`.`asset_code`
	AND `new`.`facility_code` = `existing`.`facility_code`
	AND `existing`.`current_flag` = 1
	...
	UPDATE `barrier_ods`.`merged_work_order_eis` AS `existing`
	INNER JOIN `barrier_ods`.`work_order_eis_tmp` AS `new`
	ON `new`.`Order` = `existing`.`Order`
	AND `new`.`asset_code` = `existing`.`asset_code`
	AND `new`.`facility_code` = `existing`.`facility_code`
	...
	INSERT INTO `barrier_ods`.`merged_work_order_eis`
		(	     `asset_code`
			, `facility_code`
         , `functional_loc`
			, `description`
...
SELECT      `new`.`asset_code`
	...
	FROM `barrier_ods`.`work_order_eis_tmp` AS `new`
	WHERE `new`.`action` = 'C';
