...
-- Dumping structure for procedure barrier_ods.sp_merge_work_order_nm01_mkew
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_merge_work_order_nm01_mkew`()
BEGIN
	...
	DROP TEMPORARY TABLE IF EXISTS `barrier_ods`.`work_order_nm01_mkew_tmp`;

	CREATE TEMPORARY TABLE `barrier_ods`.`work_order_nm01_mkew_tmp` AS
	SELECT `new`.*
	FROM `barrier_landing`.`work_order_nm01_mkew` `new`
	WHERE 1=0;

	ALTER TABLE  `barrier_ods`.`work_order_nm01_mkew_tmp`
	ADD COLUMN `action` VARCHAR(1) NOT NULL;


/* C */

	INSERT INTO `barrier_ods`.`work_order_nm01_mkew_tmp`
		(
          `facility_code` 
      	,`order` 
      	,`functional_loc` 
    ...
    SELECT   
   		...
   		,`new`.`maint_activ_type` 
			...
			,`new`.`system_status`   
			, 'C' AS 'action'
	FROM `barrier_landing`.`work_order_nm01_mkew` AS `new`
	INNER JOIN  `barrier_ods`.`work_order_nm01_mkew` AS `existing`
...
/*  U */
	...
	INNER JOIN  `barrier_ods`.`work_order_nm01_mkew` AS `existing`
	ON `new`.`Order` = `existing`.`order` AND `existing`.`current_flag` = 1
	WHERE `new`.`Order` NOT IN (SELECT `Order` FROM `barrier_ods`.`work_order_nm01_mkew_tmp`);
    ...
    SELECT   
   		...
   		,`new`.`maint_activ_type` 
			...
			, 'I' AS 'action'
	FROM `barrier_landing`.`work_order_nm01_mkew` AS `new`
	LEFT JOIN  `barrier_ods`.`work_order_nm01_mkew` AS `existing`
	ON `new`.`Order` = `existing`.`order` AND `existing`.`current_flag` = 1
	WHERE `existing`.`Order` IS NULL;


	/* Deal with changed rows that need to have history tracked */
	UPDATE `barrier_ods`.`work_order_nm01_mkew` AS `existing`
	INNER JOIN `barrier_ods`.`work_order_nm01_mkew_tmp` AS `new`
	...
	INSERT INTO `barrier_ods`.`work_order_nm01_mkew`
		(
          `facility_code` 
	      ,`order` 
         ,`functional_loc` 
...
SELECT     	 
	...
	FROM `barrier_ods`.`work_order_nm01_mkew_tmp` AS `new`
	WHERE `new`.`action` = 'C';
