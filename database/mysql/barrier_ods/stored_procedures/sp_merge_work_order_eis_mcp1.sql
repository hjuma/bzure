...
-- Dumping structure for procedure barrier_ods.sp_merge_work_order_eis_mcp1
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_merge_work_order_eis_mcp1`(
...
BEGIN
	...
	DROP TEMPORARY TABLE IF EXISTS `barrier_ods`.`work_order_eis_mcp1_tmp`;

	CREATE TEMPORARY TABLE `barrier_ods`.`work_order_eis_mcp1_tmp` AS
	SELECT `new`.*
	FROM `barrier_landing`.`work_order_eis_mcp1` `new`
	WHERE 1=0;

	ALTER TABLE  `barrier_ods`.`work_order_eis_mcp1_tmp`
	ADD COLUMN `action` VARCHAR(1) NOT NULL;


	INSERT INTO `barrier_ods`.`work_order_eis_mcp1_tmp`
		(
			  `functional_loc`
			, `description`
			, `order_type`
	...
	SELECT  `new`.`functional_loc`
			...
			, `new`.`coding`
			, 'C' AS 'action'
	FROM `barrier_landing`.`work_order_eis_mcp1` AS `new`
	INNER JOIN  `barrier_ods`.`work_order_eis_mcp1` AS `existing`
	...
	INNER JOIN  `barrier_ods`.`work_order_eis_mcp1` AS `existing`
	ON `new`.`Order` = `existing`.`order` AND `existing`.`current_flag` = 1
	WHERE `new`.`Order` NOT IN (SELECT `Order` FROM `barrier_ods`.`work_order_eis_mcp1_tmp`);
	...
	SELECT  `new`.`functional_loc`
			...
			, 'I' AS 'action'
	FROM `barrier_landing`.`work_order_eis_mcp1` AS `new`
	LEFT JOIN  `barrier_ods`.`work_order_eis_mcp1` AS `existing`
	ON `new`.`Order` = `existing`.`order` AND `existing`.`current_flag` = 1
	WHERE `existing`.`Order` IS NULL;


	/* Deal with changed rows that need to have history tracked */
	UPDATE `barrier_ods`.`work_order_eis_mcp1` AS `existing`
	INNER JOIN `barrier_ods`.`work_order_eis_mcp1_tmp` AS `new`
	...
	INSERT INTO `barrier_ods`.`work_order_eis_mcp1`
		(
			  `functional_loc`
			, `description`
			, `order_type`
	...
	FROM `barrier_ods`.`work_order_eis_mcp1_tmp` AS `new`
	WHERE `new`.`action` = 'C';
